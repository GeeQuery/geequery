...
import static com.github.geequery.accelerator.asm.ASMUtils.doUnwrap;
import static com.github.geequery.accelerator.asm.ASMUtils.doWrap;
import static com.github.geequery.accelerator.asm.ASMUtils.getDesc;
import static com.github.geequery.accelerator.asm.ASMUtils.getMethodDesc;
import static com.github.geequery.accelerator.asm.ASMUtils.getPrimitiveType;
import static com.github.geequery.accelerator.asm.ASMUtils.getType;
import static com.github.geequery.accelerator.asm.ASMUtils.iconst;
...
import com.github.geequery.accelerator.asm.ClassWriter;
import com.github.geequery.accelerator.asm.FieldVisitor;
import com.github.geequery.accelerator.asm.Label;
import com.github.geequery.accelerator.asm.MethodVisitor;
import com.github.geequery.accelerator.asm.Type;
...
final class ASMSwitcherGenerator extends ClassGenerator {
	...
	public byte[] generate() {
		...
		cw.visit(V1_5, ACC_PUBLIC + ACC_SUPER + ACC_FINAL, accessorName, null, "com/github/geequery/accelerator/bean/SwitchBeanAccessor", new String[] {});
		// field
		{
		...
		// //构造器
		{
			...
			mw.visitVarInsn(ALOAD, 0);
			mw.visitMethodInsn(INVOKESPECIAL, "com/github/geequery/accelerator/bean/SwitchBeanAccessor", "<init>", "()V", false);
			mw.visitTypeInsn(NEW, getType(HashSet.class));
			mw.visitInsn(DUP);

			mw.visitMethodInsn(INVOKESPECIAL, getType(HashSet.class), "<init>", "()V", false);
			...
			for (FieldInfo info : fields) {
				...
				mw.visitMethodInsn(INVOKEINTERFACE, getType(Set.class), "add", "(Ljava/lang/Object;)Z", true);
				mw.visitInsn(POP);
			}
			mw.visitInsn(RETURN);
			mw.visitMaxs(2, 2);
		...
		}
		{
			...
			mw.visitMethodInsn(INVOKEVIRTUAL, getType(String.class), "hashCode", "()I", false);
...

			{
				...
				mw.visitInsn(DUP);
				mw.visitVarInsn(ALOAD, 2);
				mw.visitMethodInsn(INVOKESPECIAL, getType(StringBuilder.class), "<init>", getMethodDesc(Void.TYPE, String.class), false);
				mw.visitLdcInsn(" is not exist in " + beanClass.getName());
				mw.visitMethodInsn(INVOKEVIRTUAL, getType(StringBuilder.class), "append", getMethodDesc(StringBuilder.class, String.class), false);
				mw.visitMethodInsn(INVOKEVIRTUAL, getType(StringBuilder.class), "toString", getMethodDesc(String.class), false);
				mw.visitMethodInsn(INVOKESPECIAL, getType(NoSuchElementException.class), "<init>", getMethodDesc(Void.TYPE, String.class), false);
				mw.visitInsn(ATHROW);
			}
			mw.visitMaxs(5, 4);
			mw.visitEnd();
		...
		}
		{
			...
			mw.visitMethodInsn(INVOKEVIRTUAL, getType(String.class), "hashCode", "()I", false); // S1
			SwitchHelper sw = new SwitchHelper();
			mw.visitLookupSwitchInsn(sw.defaultLabel, sw.hashs, sw.labels);
