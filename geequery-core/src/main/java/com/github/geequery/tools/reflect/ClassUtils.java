...
public abstract class ClassUtils {
	...
	public static String getShortName(String className) {
		Assert.isNotEmpty(className, "Class name must not be empty");
	 ...
	 * @return
	 */
	public static Set<String> getAllSuperClassNames(Class<?> clazz){
		Assert.notNull(clazz, "Class must not be null");
	 ...
	 * @return
	 */
	public static Set<String> getAllInterfaceNames(Class<?> clazz){
		Assert.notNull(clazz, "Class must not be null");
		Set<String> names = new LinkedHashSet<String>();
		if(clazz.isInterface()){
			names.add(clazz.getName());
		...
		while (clazz != null) {
			Class<?>[] interaces=clazz.getInterfaces();
			for(Class<?> intf:interaces){
	 ...
	 * @since 3.1
	 * Add by mjj,from Spring3.1.AnnotationUtils
	 */
