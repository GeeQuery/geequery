...
import com.github.geequery.accelerator.cglib.beans.BeanCopier;
...
public class PerformanceTest {
	...
	public void copyPropertiesTest() throws IllegalAccessException, InvocationTargetException, InterruptedException, ReflectionException, NoSuchMethodException {
		...
		{// 案例0：CGLib  （原生）
			...
			com.github.geequery.accelerator.cglib.beans.BeanCopier bc = com.github.geequery.accelerator.cglib.beans.BeanCopier.create(PromotionPO.class, PromotionPO.class, false);
			long startTime = System.currentTimeMillis();
			for (int i = 0; i < LOOP_NUMBER; i++) {
...
//				org.apache.commons.beanutils.PropertyUtils.copyProperties(po, source);
		...
		{// 案例4: com.github.geequery.tools.reflect.BeanUtils (含动态类创建时间)
			PromotionPO po = new PromotionPO();
			long startTime = System.currentTimeMillis();
			for (int i = 0; i < LOOP_NUMBER; i++) {
				com.github.geequery.tools.reflect.BeanUtils.copyProperties(source, po);
			}
			long endTime = System.currentTimeMillis();
			cost.put("Jef BeanUtils(include ASM)", endTime - startTime);
		}
		
		{// 案例5：com.github.geequery.tools.reflect.BeanUtils
...
//			}
		...
		{// 案例3: com.github.geequery.tools.reflect.BeanUtils (含动态类创建时间)
			PromotionPO po = new PromotionPO();
			long startTime = System.currentTimeMillis();
			for (int i = 0; i < LOOP_NUMBER; i++) {
				com.github.geequery.tools.reflect.BeanWrapper bw = new FastBeanWrapperImpl(po);
				bw.getPropertyValue("code");
				bw.getPropertyValue("ID");
				bw.getPropertyValue("field1");
				bw.setPropertyValue("code", "code123456789");
		...
		{// 案例4：com.github.geequery.tools.reflect.BeanUtils
			PromotionPO po = new PromotionPO();
			long startTime = System.currentTimeMillis();
			// 动态类生成时间不计入反射操作时间
			for (int i = 0; i < LOOP_NUMBER; i++) {
