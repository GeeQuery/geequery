...
/**
 * 提供和{@link List}接口相同的功能，但是免去了boolean类型的装箱和拆箱，从而得到性能上的改善。
 * 适用于大规模操作boolean基本类型数组的情况。
 * 
...
public final class BooleanList {
     ...
     * @see #contains(Object)
     */
    public boolean containsAll(boolean[] c){
    ...
    */
     ...
     * 使用者必须清楚对容器中数据的更改可能造成List无法正确工作。
     * 这个方法适用于用户取走数据后，List已经没有用处的场合。
     * @return
     */
    public boolean[] toArrayUnsafe(){
    	if(size==list.length){
    		return list;
    	}else{
    		boolean[] array=new boolean[size];
     ...
     */
    public void addAll(int index, boolean[] in){
    ...
    public BooleanList subList(int fromIndex, int toIndex){
    	...
    	BooleanList r=new BooleanList(len);
 ...
 public boolean retainAll(BooleanList c){
	BooleanList result=new BooleanList(Math.min(size, c.size));
 	for(int i=0;i<size;i++){
 		boolean value=list[i];
 		if(c.contains(value)){
 			result.add(value);
 		}
 	}
