...
public final class GenericUtils {
	 ...
	 * 后者可以表示带有泛型参数的类型。 如将后者转换前者
	 * getRawClass()，可能会丢失信息，此方法判断为true的情况下，可以转换为class而不丢失数据类型。
	 * 
	 * @param type
	 *            要检测的Type
	 * @return
	 * @Throws RuntimeException 输入类型必须是一个泛型Array或class
	 *         Array,如果输入类型不是一个数组类型，抛出RuntimeException。
	 */
