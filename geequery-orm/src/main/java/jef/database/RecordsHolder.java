...
public final class RecordsHolder<T extends IQueryableEntity> implements Iterable<T> {
	...
	public RecordHolder<T> get(int i) {
		ensureOpen();
		return rhs.get(i);
	}

	public RecordsHolder(ITableMetadata mm) throws SQLException {
		this.meta = mm;
		Assert.notNull(meta);
	}

	public void init(ResultSetWrapper holder, List<T> objs, DatabaseDialect profile) {
