...
public abstract class PagingIterator<T> implements Iterator<List<T>> {
	...
	/*
	 * TODO 用于报表或导出时，使用Oracle共享锁，阻止记录被插入和更改。 关于数据导出时分页的一致性问题。
	 * 一个方法是加上共享锁，这样所有的写操作都会挂起。 * 还有一个方式是使用create table as
	 ...
	 * 
	 * @param offset
	 *            从0开始。如要跳过前10条记录则输入10，查询结果将从第11条记录开始
	...
	public Page<T> getPageData() {
		Pageable pageable = PageRequest.of(page.getCurrentPage(), page.getRowsPerPage());
		return new PageImpl<T>(next(), pageable, page.getTotal());
	}
