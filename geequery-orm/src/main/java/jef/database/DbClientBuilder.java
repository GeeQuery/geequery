...
import com.github.geequery.codegen.EntityEnhancer;
...
import com.github.geequery.entity.QuerableEntityScanner;
import com.github.geequery.entity.data.DbInitHandler;
import com.github.geequery.helper.SimpleDataSource;
...
public class DbClientBuilder {
	...
	public DbClientBuilder(String dbType, String host, int port, String pathOrName, String user, String password) {
		DatabaseDialect profile = Dialects.getDialect(dbType);
		if (profile == null) {
		...
		this.dataSource = new SimpleDataSource(dbURL, user, password);
	}
	...
	public DbClientBuilder(String jdbcUrl, String user, String password) {
		this.setDataSource(new SimpleDataSource(jdbcUrl, user, password));
	}
...
//		}
		...
		// check data sources.
		if (dataSource == null) {
			throw new NullPointerException("GeeQuery has not found any datasource.");
