...
import com.github.geequery.annotation.PartitionFunction;
import com.github.geequery.annotation.PartitionKey;
...
import com.github.geequery.tools.reflect.BeanAccessorMapImpl;
...
public class DynamicMetadata extends AbstractMetadata {
	...
	public DynamicMetadata(AbstractMetadata parent, ExtensionConfig extension) {
		...
		this.containerAccessor = null;//new ExtensionAccessor(raw, extension.getName(), EfPropertiesExtensionProvider.getInstance());
	...
	protected boolean internalUpdateColumn(Field field, String columnName, ColumnType type, boolean isPk, boolean replace) {
		...
		if (isPk)
			pkFields.add(mType);
		if (mType.isLob()) {
			lobNames = com.github.geequery.tools.ArrayUtils.addElement(lobNames, oldField, jef.database.Field.class);
		}
