...
import com.github.geequery.tools.support.LangUtils;
	...
	private Entry<T, Boolean> max(T v1, Boolean c1, T v2, Boolean c2, int nullSupport, boolean closeFirst) {
		if (Objects.equal(v1, v2)) {
			if (closeFirst) {// 当值相等时，取闭区间
				return new com.github.geequery.common.Entry<T, Boolean>(v1, c1 || c2);
			} else {
				return new com.github.geequery.common.Entry<T, Boolean>(v1, c1 && c2);
			}
		} else {
			...
			if (max == v1) {
				return new com.github.geequery.common.Entry<T, Boolean>(v1, c1);
			} else {
				return new com.github.geequery.common.Entry<T, Boolean>(v2, c2);
			}
		}
	}
