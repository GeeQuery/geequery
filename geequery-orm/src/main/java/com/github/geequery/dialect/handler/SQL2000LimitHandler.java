...
public class SQL2000LimitHandler implements LimitHandler {
	public SQLBindings toPageSQL(String sql, PageLimit offsetLimit) {
		...
		if (offset == 0) {// 没有offset可以简化处理
			...
			return new SQLBindings(new StringBuilder(sql.length() + 8).append(sql).insert(index + (indexDistinct == index ? 15 : 6), " top " + offsetLimit.getLimit()).toString()
					, Arrays.asList());
		}
		return processToPageSQL(sql, offsetLimit);
	}

	private SQLBindings processToPageSQL(String sql, PageLimit offsetLimit) {
	...
	protected SQLBindings toPage(PageLimit offsetLimit, SQLServerSelectQueryBlock selectBody, SQLSelect select, String raw) {
	...
	protected SQLBindings toPage(PageLimit offsetLimit, SQLUnionQuery union,SQLSelect select, String raw) {
