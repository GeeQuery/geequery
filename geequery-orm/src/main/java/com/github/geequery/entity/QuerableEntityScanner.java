...
import com.github.geequery.dialect.type.AutoIncrementMapping.GenerationResolution;
import com.github.geequery.dialect.type.ColumnMapping;
import com.github.geequery.entity.data.DataInitializer;
...
public class QuerableEntityScanner {
	...
	private void registeEntity0(Class<?> c) {
		try {
			EntityMetadata meta = MetaHolder.getMeta(c);// 用initMeta变为强制初始化。getMeta更优雅一点
			if (meta != null) {
	 ...
	 * @throws SQLException
	 */
	private boolean doTableDDL(EntityMetadata meta, final boolean doCreateTask, final boolean refresh) throws SQLException {
		// 不管是否存在，总之先创建一次
		SessionFactory client = entityManagerFactory.getDefault();
		...
		if (!newTable) {
			client.refreshTable(meta, new MetadataEventListener() {
				public void onTableFinished(EntityMetadata meta, String tablename) {
				}

				public boolean onTableCreate(EntityMetadata meta, String tablename) {
					return doCreateTask;
				}
				...
				public boolean onColumnsCompared(String tablename, EntityMetadata meta, Map<String, ColumnType> insert, List<ColumnModification> changed,
						List<String> delete) {
					if (!allowDropColumn) {
						delete.clear();
					}
				...
				public boolean beforeTableRefresh(EntityMetadata meta, String table) {
					return true;
				}

				public void beforeAlterTable(String tablename, EntityMetadata meta, StatementExecutor conn, List<String> sql) {
				}
			});
		}
		// 检查Sequence
