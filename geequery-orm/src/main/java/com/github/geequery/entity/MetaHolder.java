...
import jef.database.meta.JoinColumnImpl;
...
import com.github.geequery.accelerator.asm.Attribute;
...
import com.github.geequery.annotation.Cascade;
import com.github.geequery.annotation.EasyEntity;
import com.github.geequery.annotation.FieldOfTargetEntity;
import com.github.geequery.annotation.Indexed;
import com.github.geequery.annotation.JoinDescription;
import com.github.geequery.annotation.JoinType;
import com.github.geequery.common.log.LogUtil;
import com.github.geequery.common.wrapper.Holder;
import com.github.geequery.core.DbUtils;
import com.github.geequery.core.config.DbCfg;
import com.github.geequery.core.config.ORMConfig;
import com.github.geequery.core.support.EntityNotEnhancedException;
...
import com.github.geequery.dialect.type.ColumnMappings;
import com.github.geequery.entity.AnnotationProvider.ClassAnnotationProvider;
import com.github.geequery.entity.AnnotationProvider.FieldAnnotationProvider;
...
public class MetaHolder {
	 ...
	 * @return
	 */
	public static EntityMetadata initMetadata(Class<?> clz, String schema, String tablename) {
		Assert.notNull(clz);
		EntityMetadata me = (TableMetadata) pool.get(clz);
	 ...
	 * @return
	 */
	public static final <T> EntityMetadata<T> getMeta(Class<T> clz) {
		Assert.notNull(clz);
		if (clz == Tuple.class) {
	...
	private static AbstractMetadata initEntity(Class<?> clz) {
		ClassAnnotationProvider annos = config.getAnnotations(clz);
		List<java.lang.reflect.Field> unprocessedField = new ArrayList<java.lang.reflect.Field>();
	...
	private static <T> EntityMetadata<T> internalProcess(Class<T> clz, List<java.lang.reflect.Field> unprocessedField, ClassAnnotationProvider annos) {
		{
		...
		EntityMetadata meta = new EntityMetadata(clz, annos);
		MeteModelFields metaFields = new MeteModelFields(clz, meta);
	...
	static class MeteModelFields {
		...
		MeteModelFields(Class<?> clz, EntityMetadata meta) {
	...
	private static void processMetaForClz(Class<?> processingClz, List<java.lang.reflect.Field> unprocessedField, TableMetadata meta,
			ClassAnnotationProvider annos, MeteModelFields metaModel) {
	...
	private static void assertFieldEnhanced(Field field, List<Field> fieldss, Class<?> processingClz) {
		...
		if (!isEnhancedProperty) {
			throw new IllegalArgumentException("Field [" + field.name() + "] may be not enhanced. Please add the enum Field [" + field.name() + "] into "
					+ processingClz.getName());
		}
	}
	...
	private static JoinPath getHint(FieldAnnotationProvider annos, AbstractMetadata meta, EntityMetadata target) {
	...
	private static JoinPath processJoin(AbstractMetadata meta, EntityMetadata target, FieldAnnotationProvider annos, JoinTable jt) {
	...
	protected static boolean processReference(AbstractMetadata meta, FieldAnnotationProvider field) {
		...
		if (field.getAnnotation(OneToOne.class) != null) {
			...
			EntityMetadata target = getTargetType(r1Vs1.targetEntity(), targetField, field, false);
		...
		if (field.getAnnotation(OneToMany.class) != null) {
			...
			EntityMetadata target = getTargetType(r1VsN.targetEntity(), targetField, field, true);
		...
		if (field.getAnnotation(ManyToOne.class) != null) {
			...
			EntityMetadata target = getTargetType(rNVs1.targetEntity(), targetField, field, false);
		...
		if (field.getAnnotation(ManyToMany.class) != null) {
			...
			EntityMetadata target = getTargetType(rNVsN.targetEntity(), targetField, field, true);
	 ...
	 * @return
	 */
	private static EntityMetadata getTargetType(Class<?> targetEntity, FieldOfTargetEntity targetField, FieldAnnotationProvider field, boolean isMany) {
		if (targetEntity != void.class) {
			if (IQueryableEntity.class.isAssignableFrom(targetEntity)) {
				...
				throw new IllegalArgumentException("The target entity type [" + targetEntity.getName() + "] for " + field.getDeclaringClass().getSimpleName()
						+ ":" + field.getName() + " is not subclass of DataObject.");
			}
		}
		if (targetField != null) {
	...
	private static JoinPath processJoin(AbstractMetadata thisMeta, EntityMetadata target, AnnotationProvider field, JoinColumn... jj) {
		List<JoinKey> result = new ArrayList<JoinKey>();

		String fieldName = field.getName();
		for (JoinColumn j : jj) {
			if (StringUtils.isBlank(j.name())) {
				throw new IllegalArgumentException("Invalid reference [" + thisMeta.getThisType().getName() + "." + fieldName
						+ "]:The field 'name' in JoinColumn is empty");
			}
			Field left = thisMeta.getField(j.name());
			Assert.notNull(left, "Invalid reference [" + thisMeta.getThisType().getName() + "." + fieldName + "]: field [" + j.name()
					+ "] not found from entity " + thisMeta.getThisType().getName());
			Field right = target.getField(j.referencedColumnName());
			if (right == null) {
				throw new NoResultException("Invalid reference [" + thisMeta.getThisType().getName() + "." + fieldName + "]: '" + j.referencedColumnName()
						+ "' is not available in " + target.getThisType().getName());
			}
	...
	private static JoinPath processJoin(EntityMetadata meta, EntityMetadata target, AnnotationProvider annos, String mappedBy) {
		...
		if (meta.getPKFields().size() != 1) {
			throw new IllegalArgumentException(meta.getSimpleName() + " cann't map to " + target.getSimpleName() + " since its primary key field count "
					+ meta.getPKFields().size());
		}
		Field left = meta.getPKFields().get(0).field();
		Field right = target.getField(mappedBy);
		if (right == null) {
			throw new IllegalArgumentException(meta.getSimpleName() + " cann't map to " + target.getSimpleName() + " since there is no field [" + mappedBy
					+ "] in target entity");
		}
		result.add(new JoinKey(left, right));
		JoinType type = JoinType.LEFT;
	...
	private static JoinKey getJoinExpress(EntityMetadata thisMeta, EntityMetadata targetMeta, String exp) {
		try {
	...
	private static Field parseField(String keyword, EntityMetadata thisMeta, EntityMetadata target) {
