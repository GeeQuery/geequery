...
import com.github.geequery.common.Callback;
import com.github.geequery.common.log.LogUtil;
import com.github.geequery.core.cache.Cache;
import com.github.geequery.core.cache.CacheDummy;
import com.github.geequery.core.config.DbCfg;
import com.github.geequery.core.config.ORMConfig;
import com.github.geequery.core.support.DbOperatorListener;
import com.github.geequery.core.support.DbOperatorListenerContainer;
import com.github.geequery.core.support.DefaultDbOperListener;
...
public class DbClient extends Session implements SessionFactory {
	...
	public Collection<String> existsTable(IQueryableEntity obj) throws SQLException {
		PartitionResult[] result = DbUtils.toTableNames(obj, null, null);
	...
	public int dropTable(ITableMetadata meta) throws SQLException {
		PartitionResult[] pr = DbUtils.toTableNames(meta, null, 4);
	...
	public int dropTable(IQueryableEntity obj) throws SQLException {
		PartitionResult[] pr = DbUtils.toTableNames(obj, null, obj.getQuery());
	...
	public void truncate(ITableMetadata meta) throws SQLException {
		...
		route = DbUtils.toTableNames(meta, null, 4);
		truncate(meta, route);
	}
	...
	public boolean createTable(IQueryableEntity obj) throws SQLException {
		...
		PartitionResult[] result = DbUtils.partitionUtil.toTableNames(meta, obj, obj.getQuery(), null, false);
	...
	public int createTable(Class<?>... cs) throws SQLException {
		...
		for (Class<?> c : cs) {
			ITableMetadata meta = MetaHolder.getMeta(c);
			PartitionResult[] result = DbUtils.toTableNames(meta, null, 2);
	...
	public void refreshTable(ITableMetadata meta, MetadataEventListener event, boolean modifyConstraint, boolean modifyIndexes) throws SQLException {
		...
		ensureOpen();
		PartitionResult[] results = DbUtils.toTableNames(meta, null, 4);
