...
import com.github.geequery.core.support.MultipleDatabaseOperateException;
import com.github.geequery.core.support.SqlLog;
...
public abstract class SelectProcessor {
	...
	public static SelectProcessor get(DatabaseDialect profile, DbClient db) {
	...
	final static class PreparedImpl extends SelectProcessor {
		...
		public CountClause toCountSql(ConditionQuery obj) throws SQLException {
			if (obj instanceof Query<?>) {
				...
				PartitionResult[] sites = DbUtils.toTableNames(query.getInstance(), myTableName, query);
	...
	public void processSelect(final QueryClause sql, final Session session, final ConditionQuery queryObj, final ResultSetContainer rs, final QueryOption option, int mustTx) throws SQLException {
	...
	public long processCount(Session session, CountClause sqls) throws SQLException {
