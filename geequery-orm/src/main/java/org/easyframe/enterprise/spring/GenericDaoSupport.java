...
import com.github.geequery.core.PagingIteratorObjImpl;
...
public abstract class GenericDaoSupport<T extends IQueryableEntity> extends BaseDao implements GenericDao<T> {
	...
	public Page<T> findAndPage(T entity, int start, int limit) {
		try {
			...
			i.setOffset(start);
			Pageable pageable=PageRequest.of(i.getCurrentPage(), i.getRowsPerPage());
			return new PageImpl<T>(i.next(), pageable, i.getTotal());
		} catch (SQLException e) {
			throw DbUtils.toRuntimeException(e);
		}
	}
	...
	public Page<T> findAndPageCascade(T entity, int start, int limit) {
		try {
			...
			i.setOffset(start);
			List<T> data = i.next();
			return new PageImpl<T>(data, PageRequest.of(i.getCurrentPage(), i.getRowsPerPage()),i.getTotal());
		} catch (SQLException e) {
			throw DbUtils.toRuntimeException(e);
		}
	}
